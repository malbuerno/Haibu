...
module.exports = (app) => {
  ...
  app.context.serverError = {
    ...
    message: "Se ha producido un error en el servicio, vuelve a intentarlo en unos minutos",
    buttons: [
      {
        text: "Cerrar",
        cssClass: "primary",
  ...
  app.context.validateVersion = (userVersion, actualVersion) => {
    const regexpSource = "^((([0-9]+)\\.([0-9]+)\\.([0-9]+)(?:-([0-9a-zA-Z-]+(?:\\.[0-9a-zA-Z-]+)*))?)(?:\\+([0-9a-zA-Z-]+(?:\\.[0-9a-zA-Z-]+)*))?)$";
    ...
    if (exp.test(userVersion)) {
      ...
      return userVersionArray[0] >= actualVersionArray[0] && userVersionArray[1] >= actualVersionArray[1] && userVersionArray[2] >= actualVersionArray[2];
    } else {
      return false;
    }
  };
